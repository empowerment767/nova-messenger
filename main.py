...
# MVP storage. PostgreSQL will replace this next.
users = {}
tokens = {}
connections = set()
...
.tabs .on{color:#fff;border-color:#7c5cff}
...
.primary:disabled{opacity:.6}.hint{color:#8995ab;text-align:center;font-size:13px;margin-top:14px}.error{color:#ff7d91;text-align:center;margin-top:10px;min-height:20px}
...
'<form id="authForm">'+
'<input id="nameInput" placeholder="Ваше имя" style="display:none">'+
'<input id="usernameInput" placeholder="Логин" autocomplete="username" required pattern="[A-Za-z0-9_]{3,30}" title="3–30 символов: латинские буквы, цифры и _">'+
'<input id="passInput" type="password" placeholder="Пароль (минимум 6 символов)" minlength="6" required autocomplete="current-password">'+
'<button class="primary" id="submitBtn" type="submit">Войти</button></form>'+
'<div class="error" id="errorBox"></div><div class="hint">Без email и без подтверждения • Nova MVP</div>'+
'</div></div>';
...
regTab.addEventListener("click",()=>setMode(true));
...
authForm.addEventListener("submit",async e=>{
 ...
 try{
   const body=registerMode
     ? {name:nameInput.value.trim(),username:usernameInput.value.trim(),password:passInput.value}
     : {username:usernameInput.value.trim(),password:passInput.value};
...
if(!response.ok){localStorage.removeItem("novaToken");auth();return;}
const me=await response.json();
root.innerHTML=
'<div class="chatapp"><aside class="sidebar"><div class="brand">💬 <span>NOVA</span></div>'+
'<input id="searchInput" placeholder="Поиск пользователей">'+
'<div class="user">👤 '+escapeHtml(me.name)+'<br><small>@'+escapeHtml(me.username)+'</small></div></aside>'+
'<main class="main"><header class="head"><b>Nova Messenger</b><button id="logoutBtn" type="button">↪</button></header>'+
'<section class="msgs" id="messages"><div class="welcome">Nova подключён.<br>Откройте этот адрес на другом телефоне<br>и войдите другим аккаунтом.</div></section>'+
...
document.getElementById("logoutBtn").onclick=()=>{localStorage.removeItem("novaToken");location.reload()};
const messages=document.getElementById("messages"), input=document.getElementById("messageInput");
let ws;
function connect(){
 ...
 ws.onmessage=e=>{
   ...
   if(data.type==="message"){
     const el=document.createElement("div");el.className="msg"+(data.mine?" mine":"");
     el.textContent=data.text;messages.appendChild(el);messages.scrollTop=messages.scrollHeight;
   }
 };
...
document.getElementById("chatForm").onsubmit=e=>{
 ...
 if(text&&ws&&ws.readyState===WebSocket.OPEN){ws.send(JSON.stringify({text}));input.value=""}
};
connect();
}
...
async def register(x: Register):
    ...
    if len(username)<3 or len(username)>30:
        raise HTTPException(400,"Логин: от 3 до 30 символов")
    if any(c not in "abcdefghijklmnopqrstuvwxyz0123456789_" for c in username):
        raise HTTPException(400,"Логин может содержать только латинские буквы, цифры и _")
    if username in users:
        raise HTTPException(409,"Этот логин уже занят")
    if len(x.password)<6:
        raise HTTPException(400,"Пароль должен быть минимум 6 символов")
    users[username]={"name":name[:50],"password":pw_hash(x.password)}
    token=secrets.token_urlsafe(32);tokens[token]=username
...
async def me(authorization: str | None = Header(default=None)):
    ...
    username=tokens.get(authorization[7:])
    if not username or username not in users:
        raise HTTPException(401,"Сессия недействительна")
    return {"name":users[username]["name"],"username":username}
